...
// PA4 code goes here....
Pokemon::Pokemon(string name, double speed, double hp, double phys_dmg, double magic_dmg, double def, int in_id, char in_code, Point2D in_loc) : GameObject(‘P’, id, in_loc)
{
	stamina = 20;
	health = hp;
	store_health = health;
...
void Pokemon::TakeHit(double phys_dmg, double magic_dmg, double def)
{
	...
	{
		double damage = (100.0 - defense) / 100 * magic_dmg;
		this->health -= damage;
	}
}
...
void Pokemon::Attack(int AttackNum)
{
	vector <string> att(4);
	att[0] = "ThunderShock";
	att[1] = "ThunderWave";
